...
def montar_autorizacao(empresa_razao_social, empresa_cnpj, funcionarios_lista_nomes_e_cpfs):
    ...
    titulo.alignment = 1
    titulo_estilo = titulo.add_run('\nAutorização\n\n')
    titulo_estilo.bold = True
    titulo_estilo.underline = True
    titulo_estilo.font.name = 'Calibri'
    titulo_estilo.font.size = Pt(18) 

    paragrafo = document.add_paragraph(f"\nA empresa ")
    paragrafo.alignment = 4
    paragrafo.add_run(empresa_razao_social).bold = True 
    paragrafo.add_run(f", CNPJ {empresa_cnpj}, autoriza os funcionários:")
    for index, nome_e_cpf in enumerate(funcionarios_lista_nomes_e_cpfs): #adiciona inf. funcionarios
        nome_funcionario = document.add_paragraph(f"{str(index+1)} - {nome_e_cpf[0]}, CPF {nome_e_cpf[1]}")
        nome_funcionario.paragraph_format.left_indent = Cm(2)
    
    document.add_paragraph("a retirar talões de cheque que forem a esta endereçados.\n\n")
    document.add_paragraph("\nCidade da Justiça, 04 de Maio de 1984.\n\n")
    linha_assinatura = document.add_paragraph("___________________________________")
    linha_assinatura.alignment = 1
    razao_social_empresa = document.add_paragraph()
    razao_social_empresa.alignment = 1
    razao_social_empresa.add_run(f"{empresa_razao_social}").bold = True

    document.save('demo.docx')
    
    print("Documento salvo com sucesso! Confira o arquivo 'demo.docx'.")
...
montar_autorizacao(empresa_razao_social, empresa_cnpj,
                     funcionarios_lista_nomes_e_cpfs)
